...
class WorkspaceEndpoint extends Endpoint {
  ...
  Future<List<Workspace>> getWorkspacesByUser(Session session, {required int userId}) async {
    List<Workspace> workspaces = await Workspace.find(session, where: (w) => w.userId.equals(userId));
    for(int i = 0; i < workspaces.length; i++) {
      List<Member> members = (await Member.find(
          session, where: (m) => m.workspaceId.equals(workspaces[i].id)));
  ...
  Future<List<Board>> getBoardsByWorkspace(Session session, {required int workspaceId}) async {
    List<Board> boards = await Board.find(session, where: (b) => b.workspaceId.equals(workspaceId));
    return boards;
  }
